...
let apiDataGet = async () => {
    let data = getProjectData(288)       // Change Project_ID Integer Value only Ex:1131
        .then((data) => {
            phone_no = data.result.magnet_id ? data.result.magnet_number : data.result.phone;
...
async function saveLead(name, email,country_code, number, pref) {
    if ($('.dropnOtp').val() == '' && parseInt(number)) {
        const otpData = {
        ...
        $.post("https://api.homesfy.in/api/users/send_otp", otpData, function (data, status) {
            if (status == "success") {
                $('.dropnOtp').val(data.otp);
            }
        });
    }

    if (($('.dropnOtp').val() != '' && ($('.dropnOtp').val() == $('.FillOtp').val() || $('.dropnOtp').val() == $('.FillOtp1').val() || $('.dropnOtp').val() == $('.FillOtp2').val()))) {
